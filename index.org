...
**** Level 4: Adding your classes to ROOT
Sometimes it is useful to add your own developed classes to ROOT
(so they can be used in ROOT macros/PyROOT scripts or so they can
be stored in a ROOT file). For example you may have a class of the form
(in a file called anEvent.h, for example):
#+BEGIN_SRC cpp
#ifndef anEvent_h
#define anEvent_h
...
#endif
#+END_SRC
The ROOT developers suggest inheriting from TObject when using your own classes in shared libraries with ROOT.
We also need anEvent.cxx:
#+BEGIN_SRC cpp
#include "anEvent.h"
...
void anEvent::set_eventID(const int eid) { _eventID = eid; }
void anEvent::set_eventEnergy(const double e) { _eventEnergy = e; }
...
#+BEGIN_SRC sh
rootcint -f anEventDictionary.cxx -c anEvent.h LinkDef.h
#+END_SRC
This generates anEventDictionary.cxx and anEventDictionary.h. Now you can compile
the source for your class and the new dictionary source. Then finally link them together
in a library:
#+BEGIN_SRC sh
g++ -fPIC -c anEvent.cxx `root-config --cflags`
g++ -fPIC -c anEventDictionary.cxx `root-config --cflags`
g++ -shared -o libanEvent.so anEvent.o anEventDictionary.o `root-config --glibs`
#+END_SRC
Now by loading the library libanEvent.so you can use your class within ROOT. For example,
the macro:
#+BEGIN_SRC cpp
{
  gSystem->Load("libanEvent");
  anEvent a;
  a.set_eventID(42);
  std::cout << a.eventID() << std::endl;
}
#+END_SRC
will simply print 42. Or the PyROOT script:
#+BEGIN_SRC python
import ROOT
ROOT.gSystem.Load('libanEvent')
