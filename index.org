...
is too much of a review to be useful as a primary source.  Griffiths
...
The task: plot Monte Carlo predictions against data. This seems to be
another topic that is confusing until its not and no one documents it
once its no longer confusing.  Here's my stab at it:
...
experiment. How do we compare this to Monte Carlo? There, the computer
counts up the number of events that matched the criteria, and what we
want is
\begin{equation}
N_{exp}=W N_{MC}
\end{equation}
...
order as $N_{D}$.  There is a cross section associated with the MC
dataset, so we can write
\begin{equation}
...
multiply $\sigma_{MC}$ by the k factor, and the calculation is updated
to the newest NLO prediction.
