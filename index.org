...
If you find this guide entertaining, but uninformative, then please
send a bottle of Old Janx Spirit to The Editor. The Guide is not
intended to be read linearly.  Jump to the section you need, and if
...
- [[./index.pdf][PDF of The Guide]] if you prefer that sort of thing, though its of
...
is a marvel.  You are a stone mason, and you are building a
cathedral.  
...
- [[http://root.cern.ch/drupal/content/roofit][RooFit]]: Software for likelihood fitting (caveat: there are some
  strange namespace issues with this code)
...
**** TTrees
***** Drawing trees
When you call =TTree::Draw=, the order of the axes is "z:y:x"
rather than the expected "x:y:z" 
***** Caching trees
...
you're just splitting it in half, it would look like:
#+BEGIN_SRC cpp
TTree* outTreeA=tree->CopyTree("Entry$%2==0");
TTree* outTreeB=tree->CopyTree("Entry$%2==1");
#+END_SRC
**** TH1
...
numbers) represent the order in which function was called with the
most recent call at the top of the list.  Since this code was running
a single thread, there is a only one stack trace, and then there is
the abbreviated stack trace. If there were multiple threads, there
would be a trace for each thread. Typically the fastest route to a
user called function is to look at the portion:
#+BEGIN_EXAMPLE
The lines below might hint at the cause of the crash.
If they do not help you then please submit a bug report at
http://root.cern.ch/bugs. Please post the ENTIRE stack trace
...
here. 
**** Flat text file
On option is to keep a (well organized) text file for recording what
